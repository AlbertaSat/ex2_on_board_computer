...
void CLIhandler(void){
    ...
    //TODO: ensure that it is clear for SPI and UART if numbering is for stack or for microcontroller

    /*
     * Filesystem commands to support in future (same syntax as linux terminal):
     * format
     * mount
     * mkdir
     * cd
    ...
    while(1){
        ...
        sciSend(sciREG4, strlen("s - SPI test\r\n"), (unsigned char *)"s - SPI test\r\n");
        sciSend(sciREG4, strlen("u - UART test\r\n"), (unsigned char *)"u - UART test\r\n");
        sciSend(sciREG4, strlen("i - I2C test\r\n"), (unsigned char *)"i - I2C test\r\n");
...
//            case 's': {
//                unsigned char cmd2;
//                sciSend(sciREG4, strlen("Testing Stack SPI\r\n"), (unsigned char *)"Testing Stack SPI\r\n");
//                sciSend(sciREG4, strlen("NOTE: SPI slave tests not supported currently\r\n"), (unsigned char *)"NOTE: SPI slave tests not supported currently");
...
//                    if(cmd2 == 't'){
//                        unsigned char cmd3;
//                        sciSend(sciREG4, strlen("Press 1 or 2 to enable SPI1 or SPI2 tx, respectively\r\n"), (unsigned char *)"Press 1, 2, or 3 to enable UART1, UART2, or UART3 tx, respectively\r\n");
//                        sciSend(sciREG4, strlen("NOTE: UART4 cannot be tested since it is used for serial terminal\r\n"), (unsigned char *)"NOTE: UART4 cannot be tested since it is used for serial terminal\r\n");
//                        while(1){
//                            cmd3 = sciReceiveByte(sciREG4);
//                            if(cmd3 == '1'){
//                                UARTTxTest(sciREG1);
//                                sciSend(sciREG4, strlen("1 kB sent over UART1\r\n"), (unsigned char *)"1 kB sent over UART1\r\n");
//                                break;
//                            }
//                            else if(cmd3 == '2'){
//                                UARTTxTest(sciREG2);
...
//                        unsigned char cmd3;
//                        sciSend(sciREG4, strlen("Press 1, 2, or 3 to enable UART1, UART2, or UART3 rx, respectively\r\n"), (unsigned char *)"Press 1, 2, or 3 to enable UART1, UART2, or UART3 rx, respectively\r\n");
//                        sciSend(sciREG4, strlen("NOTE: UART4 cannot be tested since it is used for serial terminal\r\n"), (unsigned char *)"NOTE: UART4 cannot be tested since it is used for serial terminal\r\n");
//                        while(1){
//                            cmd3 = sciReceiveByte(sciREG4);
//                            if(cmd3 == '1'){
...
//                                sciSend(sciREG4, strlen("Bytes received over UART1 = "), (unsigned char *)"Bytes received over UART1 = ");
//                                sciSend(sciREG4, 3, (unsigned char *)recvbytesstr);
//                                sciSend(sciREG4, strlen("\r\n"), (unsigned char *)"\r\n");
//                                break;
//                            }
//                            else if(cmd3 == '2'){
...
//            }
            case 'u': {
                ...
                while(1){
                    ...
                    if(cmd2 == 't'){
                        unsigned char cmd3;
                        sciSend(sciREG4, strlen("Press 1, 2, or 3 to enable UART1, UART2, or UART3 tx, respectively\r\n"), (unsigned char *)"Press 1, 2, or 3 to enable UART1, UART2, or UART3 tx, respectively\r\n");
                        sciSend(sciREG4, strlen("NOTE: UART4 cannot be tested since it is used for serial terminal\r\n"), (unsigned char *)"NOTE: UART4 cannot be tested since it is used for serial terminal\r\n");
                        while(1){
                            cmd3 = sciReceiveByte(sciREG4);
                            if(cmd3 == '1'){
                ...
                while(1){
                    ...
                    if(cmd2 == 'o'){
                        ...
                        while(1){
                            ...
                            sciSend(sciREG4, 1, (unsigned char *)(GPIOGetBit(1)+48));//weird addition needed to convert to char
                            sciSend(sciREG4, strlen("         "), (unsigned char *)"         ");
                            sciSend(sciREG4, 1, (unsigned char *)(GPIOGetBit(8)+48));//weird addition needed to convert to char
                            sciSend(sciREG4, strlen("         "), (unsigned char *)"         ");
                            sciSend(sciREG4, 1, (unsigned char *)(GPIOGetBit(10)+48));//weird addition needed to convert to char
                            sciSend(sciREG4, strlen("         "), (unsigned char *)"         ");
                            sciSend(sciREG4, 1, (unsigned char *)(GPIOGetBit(11)+48));//weird addition needed to convert to char
                            sciSend(sciREG4, strlen("\r\n"), (unsigned char *)"\r\n");
            ...
            case 'c': {
                ...
                while(1){
                    ...
                    if(cmd2 == 'r'){
                        ...
                        sciSend(sciREG4, strlen("Frames expected = 125"), (unsigned char *)"Frames expected = 125");
            ...
            case 'r': {
                ...
                while(1){
                    ...
                    if (cmd2 == 'r'){
                        ...
                        sciSend(sciREG4, strlen("RTC time (seconds) = \r\n"), (unsigned char *)"RTC time (seconds) = \r\n");
                        sciSend(sciREG4, 1, (unsigned char *)&secs);//does this convert to ASCII unintentionally?
            ...
            case 'v': {
                ...
                while(1){
                    int j;
                    for(j=0;j<8;j++){
                        sciSend(sciREG4, 1, (unsigned char *)(switchstatus(j)+48));//weird addition needed to convert to char
                        sciSend(sciREG4, strlen("      "), (unsigned char *)"      ");
                    }
                    sciSend(sciREG4, strlen("\r\n"), (unsigned char *)"\r\n");
                    for(i=0;i<10000000;i++);
            ...
            case 'm': {
                ...
                while(1){
                    ...
                    //fill in
                    if (gioGetBit(hetPORT2, 0) == 0){//obvs make this an interrupt thing in the future instead of polling
