...
#include <../../src/drivers/I2C/cspI2C_EFM32_M3.h>

static int csp_i2c_handle = 1;
...
int csp_i2c_tx(csp_iface_t * interface, csp_packet_t * packet, uint32_t timeout) {
	...
	/* enqueue the frame */
	if (i2c_send(csp_i2c_handle, frame, timeout) != CSP_ERR_NONE)
...
int csp_i2c_init(uint8_t addr, int handle, int speed) {
	...
	if (i2c_init(addr, handle, speed) != CSP_ERR_NONE){
		puts("diver error");
		return CSP_ERR_DRIVER;
	}
